...
function draw() {
  const gradient = ctx.createLinearGradient(0, 0, 0, H); gradient.addColorStop(0, "#070b24"); gradient.addColorStop(.6, "#111342"); gradient.addColorStop(1, "#260f3b"); ctx.fillStyle = gradient; ctx.fillRect(0, 0, W, H);
  ctx.fillStyle = "#d8fbff"; starsBg.forEach(s => { const x = (s.x - camera * .12 + W) % W; ctx.globalAlpha = .3 + (s.r / 4); ctx.beginPath(); ctx.arc(x, s.y, s.r, 0, Math.PI * 2); ctx.fill(); }); ctx.globalAlpha = 1;
  ctx.save(); ctx.translate(-camera, 0);
  ctx.fillStyle = "#0b1231"; for (let x = 0; x < worldWidth; x += 150) { const h = 80 + (x * 17) % 110; ctx.fillRect(x, 470 - h, 100, h); ctx.fillStyle = "#18305a"; for (let wy = 470 - h + 18; wy < 455; wy += 22) { ctx.fillRect(x + 14, wy, 8, 4); ctx.fillRect(x + 34, wy, 8, 4); ctx.fillRect(x + 64, wy, 8, 4); } ctx.fillStyle = "#0b1231"; }
  ctx.strokeStyle = "#153b69"; ctx.lineWidth = 1; for (let x = -200; x < worldWidth + 400; x += 80) { ctx.beginPath(); ctx.moveTo(x, 470); ctx.lineTo(x + (x - worldWidth / 2) * .2, 540); ctx.stroke(); } for (let y = 480; y < 540; y += 16) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(worldWidth, y); ctx.stroke(); }
  platforms.forEach(p => { ctx.shadowColor = "#00f5ff"; ctx.shadowBlur = 12; ctx.fillStyle = "#182b50"; ctx.fillRect(p.x, p.y, p.w, p.h); ctx.shadowBlur = 0; ctx.fillStyle = "#00f5ff"; ctx.fillRect(p.x, p.y, p.w, 4); ctx.fillStyle = "#ff2bd6"; ctx.fillRect(p.x, p.y + p.h - 3, p.w, 3); });
  collectibles.forEach(c => { if (!c.taken) { const pulse = 9 + Math.sin(Date.now() / 180 + c.x) * 2; ctx.shadowColor = "#ffe178"; ctx.shadowBlur = 16; ctx.fillStyle = "#ffe178"; ctx.beginPath(); ctx.moveTo(c.x, c.y - pulse); ctx.lineTo(c.x + pulse * .7, c.y); ctx.lineTo(c.x, c.y + pulse); ctx.lineTo(c.x - pulse * .7, c.y); ctx.closePath(); ctx.fill(); ctx.shadowBlur = 0; } });
  enemies.forEach(e => { if (e.alive) { ctx.shadowColor = "#ff2bd6"; ctx.shadowBlur = 14; ctx.fillStyle = "#d91e91"; ctx.fillRect(e.x, e.y, e.w, e.h); ctx.shadowBlur = 0; ctx.fillStyle = "#0b1231"; ctx.fillRect(e.x + 6, e.y + 9, 5, 5); ctx.fillRect(e.x + 17, e.y + 9, 5, 5); ctx.fillStyle = "#ff5be5"; ctx.fillRect(e.x - 4, e.y - 5, e.w + 8, 4); } });
  ctx.shadowColor = "#00f5ff"; ctx.shadowBlur = 20; ctx.fillStyle = "#b8faff"; ctx.fillRect(4000, 310, 8, 160); ctx.shadowBlur = 0; ctx.fillStyle = "#ff2bd6"; ctx.beginPath(); ctx.moveTo(4008, 315); ctx.lineTo(4080, 335); ctx.lineTo(4008, 355); ctx.fill();
  if (!(player.invincible > 0 && Math.floor(player.invincible / 5) % 2 === 0)) { ctx.shadowColor = "#00f5ff"; ctx.shadowBlur = 18; ctx.fillStyle = "#00f5ff"; ctx.fillRect(player.x, player.y, player.w, player.h); ctx.shadowBlur = 0; ctx.fillStyle = "#07152f"; ctx.fillRect(player.x + 7, player.y + 9, 6, 6); ctx.fillRect(player.x + 19, player.y + 9, 6, 6); ctx.fillStyle = "#ff2bd6"; ctx.fillRect(player.x - 3, player.y + 28, player.w + 6, 7); }
  if (player.attackTime > 0) { ctx.shadowColor = "#ffe178"; ctx.shadowBlur = 14; ctx.strokeStyle = "#fff2a8"; ctx.lineWidth = 5; ctx.beginPath(); const a = attackBox(); ctx.arc(a.x + 16, a.y + 14, 23, player.facing > 0 ? -.8 : 2.3, player.facing > 0 ? .8 : 3.8); ctx.stroke(); ctx.shadowBlur = 0; }
  ctx.restore();
}
